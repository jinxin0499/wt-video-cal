...
def _make_manager_summary() -> ManagerSummary:
    record1 = VideoRecord(
        "acct1", "v1", "Product A", Decimal("100"), 5, 6, Currency.USD, "f", Decimal("95")
    )
    record2 = VideoRecord(
        "acct2", "v2", "Product B", Decimal("200"), 10, 12, Currency.GBP, "f", Decimal("180")
    )
    ...
    cr2 = CommissionResult(
        ...
        region="英国",
        manager="张三",
        exchange_rate=Decimal("9.20"),
        gmv_cny=Decimal("1840"),
        profit_margin=Decimal("0.30"),
        profit_cny=Decimal("552"),
    ...
    acct1 = AccountSummary(
        ...
        total_orders=5,
        total_items_sold=6,
        total_gmv_cny=Decimal("725"),
        total_profit_cny=Decimal("217.50"),
        total_commission=Decimal("10.88"),
        details=[cr1],
    )
    acct2 = AccountSummary(
        ...
        total_orders=10,
        total_items_sold=12,
        total_gmv_cny=Decimal("1840"),
        total_profit_cny=Decimal("552"),
        total_commission=Decimal("27.60"),
        details=[cr2],
    )
    ...
    return ManagerSummary(
        ...
        total_orders=15,
        total_items_sold=18,
        total_gmv_cny=Decimal("2565"),
        total_profit_cny=Decimal("769.50"),
        total_commission=Decimal("38.48"),
        accounts={"acct1": acct1, "acct2": acct2},
        creator_side_gmv_usd=Decimal("403.79"),
        gmv_diff_usd=Decimal("50.00"),
        adjustment_commission_cny=Decimal("5.44"),
    )
...
class TestWriteManagerReport:
    def test_creates_file(self, tmp_path: Path) -> None:
        ...
        path = write_manager_report(ms, tmp_path, "2026-01", exchange_rate_usd=Decimal("7.25"))
        assert path.exists()
    ...
    def test_summary_sheet(self, tmp_path: Path) -> None:
        ...
        # Check header
        assert rows[0][0] == "账号"
        assert rows[0][8] == "GMV(USD约)"
        assert rows[0][12] == "差额补算(CNY)"
        # Check total row
        assert rows[3][0] == "合计"
        assert rows[3][2] == 15  # total orders
        assert rows[3][8] == 353.79
        assert rows[3][10] == 403.79
        assert rows[3][11] == 50
        assert rows[3][12] == 5.44
        assert rows[3][13] == 43.92
...
class TestWriteOverviewReport:
    ...
    def test_manager_summary_sheet(self, tmp_path: Path) -> None:
        ...
        assert rows[1][5] == 403.79
        assert rows[1][6] == 50
        assert rows[1][7] == 5.44
        assert rows[1][9] == 43.92
        assert rows[2][4] == 353.79
        assert rows[2][7] == 5.44
        assert rows[2][9] == 43.92
    ...
    def test_account_detail_sheet(self, tmp_path: Path) -> None:
        ...
        ws = wb["账号明细"]
        rows = list(ws.iter_rows(values_only=True))
        # Header + 2 accounts + total
        assert len(rows) == 4
        assert rows[0][9] == "GMV(USD约)"
        assert rows[3][0] == "合计"
        assert rows[3][9] == 353.79

    def test_low_margin_review_sheet(self, tmp_path: Path) -> None:
        ms = _make_manager_summary()
        managers = {"张三": ms}
        review_items = [
            LowMarginReviewItem(
                source_file="shop.xlsx",
                manager="张三",
                account="acct1",
                region="美国",
                video_id="v5000",
                product_name="5000mAh Charging Kit",
                currency="USD",
                video_gmv=Decimal("120"),
                orders=1,
                items_sold=1,
                unit_price_original=Decimal("120"),
                unit_price_cny=Decimal("870"),
                matched_pattern="5000",
                rule_margin=Decimal("0.15"),
        ...
        rows = list(ws.iter_rows(values_only=True))
        assert len(rows) == 2
        assert rows[0][0] == "来源文件"
        assert rows[1][4] == "v5000"
        assert rows[1][15] == "件单价CNY超过阈值"
